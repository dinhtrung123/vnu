...
if(huyenId!=0){
	...
	String _message = "<liferay-ui:icon image=\"check\" url=\"\" message=\"\"/>" ;

	
	
	PortletURL cmtVerifyURL = renderResponse.createRenderURL();
...
<portlet:actionURL name="xacMinh" var="xacMinhURL">	
...
<aui:model-context bean="<%= account1 %>" model="<%= Account.class %>" />
			...
			<aui:col columnWidth="100">
					<aui:col columnWidth="50" >
						...
						<aui:col columnWidth="60" last="true">
							...
							<aui:input label='' name="ngaySinh" type="text"  disabled="true"  style="width:95%" inlineLabel="true"  value="<%=ngaySinh %>"/> 
						</aui:col>
					</aui:col>
					 ...
					 JSONObject doVerify = JSONFactoryUtil.createJSONObject(photo.getMessage());
					  ...
					  for(AccountEnum env : AccountEnum.values()) {
				    	if(doVerify.getBoolean(env.name()) ) {
				    	    	verifyField = verifyField + LanguageUtil.get(request, env.name()) + ", "; 
	...
	<aui:form name="fm" action="<%=xacMinhURL %>" method="post">
		<input type="hidden" name="<portlet:namespace/>accountId" />
		<aui:input name="<%=Constants.CMD%>" type="hidden" />
		<aui:input name="ipLevel" type="hidden" />
			<aui:button-row >
					<aui:button type="button" onClick="<%= popup_cmt_verify %>" value="Upload" cssClass="btn-primary"/>
<%-- 					<aui:button type="button" onClick="<%= popup_level_verify %>" value="IpLevel" cssClass="btn-primary"/>
 --%>					<aui:button name="openIpLevelSelectorButton" value="Xác minh" cssClass="btn-primary"/>
<%-- 					<aui:button type="button" value="Xác minh" 	onClick='<%= renderResponse.getNamespace() + "saveXacMinh()" %>' cssClass="btn-primary" /> --%>
					<aui:button type="button" value="Không xác minh được" 	onClick='<%= renderResponse.getNamespace() + "saveKhongXacMinh()" %>' cssClass="btn-primary"/>
...
<aui:script>
   ...
   function popup_level_verify(url){
	...
	function <portlet:namespace />saveXacMinh() {
		document.<portlet:namespace />fm.<portlet:namespace /><%= Constants.CMD %>.value = "<%= Constantes.Status.DA_XAC_MINH %>";
		submitForm(document.<portlet:namespace />fm);
	}

	function <portlet:namespace />saveKhongXacMinh() {
		document.<portlet:namespace />fm.<portlet:namespace /><%= Constants.CMD %>.value = "<%= Constantes.Status.KHONG_XAC_MINH %>";
		submitForm(document.<portlet:namespace />fm);
	}	
</aui:script>
...
A.one('#<portlet:namespace />openIpLevelSelectorButton').on(
        'click',
		function(event) {
			Liferay.Util.selectEntity(
				{
					...
					id: '<portlet:namespace />selectIpLevel',
					title: '<liferay-ui:message key="select-ip-level" />',
					uri:'<portlet:renderURL windowState="<%= LiferayWindowState.POP_UP.toString() %>"><portlet:param name="mvcPath" value="/xacminhportlet/popup-iplevel-verify.jsp" /><portlet:param name="accountid" value="<%=String.valueOf(accountId)%>" /></portlet:renderURL>' 
				},
				function(event) {
					document.<portlet:namespace />fm.<portlet:namespace />ipLevel.value =event.ipLevel;	
					document.<portlet:namespace />fm.<portlet:namespace /><%= Constants.CMD %>.value = "<%= Constantes.Status.DA_XAC_MINH %>";
					submitForm(document.<portlet:namespace />fm);				
				}
			);
...
<aui:script>
     Liferay.provide(window, 'updateDataFromPopUp', function(_message) {
    		...
    		if (json.fullname=="true"){
    			AUI().one('#verifiaction_hoten').html('Đã xác minh');
    		}
