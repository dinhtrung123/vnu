...
import dtt.vn.user.common.utils.CommonConstants;
import dtt.vn.user.common.utils.DateUtil;
import dtt.vn.vnpost.service.util.RoleConstantsExt;
...
public class UploadFileUtils {
	...
	public static long isExitsOrCreateFolder(long folderRootId, PortletRequest request) throws Exception {
		...
		User user = getAdmin();
		Folder folder = null;
		long roleId = getRoleById(companyId, RoleConstantsExt.USER).getRoleId();
		if (user != null) {
				...
				if (folder != null) {
					...
					if (folder != null) {
						if (!hasPermissions(folder.getFolderId(), roleId)) {
							setFolderPermissions(folder, RoleConstantsExt.USER);
						}
